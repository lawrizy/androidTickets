...
public class CategorieIncidentDAO {
    ...
    {
        List<CategorieIncident> result = new ArrayList<>();
        for (CategorieIncident c : listCategorie) {
            if (c.getFk_parent() != 0)
                result.add(c);
        }
        return result;
...
//        return CursorToCategorieIncidentList(c);
    }
...
//        Cursor c = db.query("w3sys_categorie_incident", new String[]{"id_categorie_incident", "label"}, " fk_parent is null", null, null, null, null);
//        return CursorToCategorieIncidentList(c);
    }
